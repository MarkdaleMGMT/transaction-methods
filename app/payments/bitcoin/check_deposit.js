...
const { transfer_btc } = require('./util')
const { bitcoinColdStorage } = require('../../../global_config')
...
module.exports = async function check_deposit(username, investment_id){
  ...
  console.log("bitcoinColdStorage ",bitcoinColdStorage);
  let status=[]
  ...
  for(let i=0; i< result.length; i++){
    ...
    if(result[i].confirmations >= 6){
      ...
      transferStatus = await transfer_btc(input_tx_id, vout, amount, deposit_address, bitcoinColdStorage);
