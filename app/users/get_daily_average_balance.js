...
const {
  ...
  get_all_investments
} = require("../models").investment_model;
 ...
 */
module.exports = async function daily_average_balance_api(req, res) {
  let username = req.body.username;
  let time_period_days = req.body.time_period_days;
  let chart = req.body.chart;

  try {
    console.log("inside get daily average average balance");
    let balance_history = await get_daily_average_balance(
      username,
      time_period_days,
      chart
    );
...
async function get_daily_average_balance(
  username,
  time_period_days,
  chart = false
) {
  ...
  for (let i = 0; i < user_accounts.length; i++) {
    ...
    let account_balance_history = await get_balance_history(
      user_account,
      time_period_days,
      chart,
      investment_map[user_account.investment_id]
    );
    user_balance_history.push({
      account_id: user_account.account_id,
      investment_id: user_account.investment_id,
      investment_name:
        investment_map[user_account.investment_id]["investment_name"],
